...
class StaffGateway(private val service: StaffService) {
    ...
    fun create(auth: Authentication, request: StaffMemberChangeRequest) = auth.checkPermission(PERMISSION_SETTINGS_ADMIN) {
        service.create(request)
    }

    fun update(auth: Authentication, id: Long, request: StaffMemberChangeRequest) = auth.checkPermission(PERMISSION_SETTINGS_ADMIN) {
